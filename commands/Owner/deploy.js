...
module.exports = class Deploy extends Command {
  ...
  async run(message) {
    ...
    const [guildCommands, globalCommands] = this.client.slashCommands.partition((c) => c.guildOnly);
    ...
    await this.client.guilds.cache.get(message.guild.id)?.commands.set(guildCommands.map((c) => c.commandData));

    // Then set the global commands like this.
    await this.client.application?.commands
      .set(globalCommands.map((c) => c.commandData))
