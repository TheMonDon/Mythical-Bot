...
class Nickname extends Command {
  ...
  async run (msg, text) { // eslint-disable-line no-unused-vars
    ...
    if (text[0]) {
      const infoMem = msg.mentions.members.first() || server.members.cache.find(m => m.id === `${text[0]}`) || server.members.cache.find(m => m.displayName.toUpperCase() === `${text[0].toUpperCase()}`) || server.members.cache.find(m => m.user.username.toUpperCase() === `${text[0].toUpperCase()}`) || server.members.cache.find(m => m.user.username.toLowerCase()
        .includes(`${text[0].toLowerCase()}`));

      if (!infoMem) return msg.channel.send('Please supply a proper member.');
      if (infoMem.id === server.owner.id) return msg.channel.send('Only the server owner of the server can change their nickname.');
      ...
      msg.channel.send(`Incorrect Usage: ${p}Nick <user> (nickname)`);
    }
  }
}
