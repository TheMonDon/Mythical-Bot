...
exports.commandData = new SlashCommandBuilder()
  ...
  .addIntegerOption((option) =>
    option.setName('bet').setDescription('The amount of money you want to bet').setRequired(true),
  );
...
exports.run = async (interaction) => {
  ...
  if (bet === Infinity) {
    return interaction.client.util.errorEmbed(interaction, "You can't bet infinity.", 'Invalid bet');
  }
  if (isNaN(bet)) return interaction.client.util.errorEmbed(interaction, 'Bet amount must be a number', 'Invalid Bet');
  if (bet < 1)
    return interaction.client.util.errorEmbed(interaction, `You can't bet less than ${currencySymbol}1`, 'Invalid Bet');
  if (BigInt(bet) > cash) {
    return interaction.client.util.errorEmbed(interaction, "You can't bet more cash than you have", 'Invalid Bet');
  }
  ...
  const messageEmbed = await interaction.editReply({ embeds: [cardEmbed()], components: [row] });
  ...
  while (!gameOver) {
    const collected = await Promise.race([
      interaction.channel
        .awaitMessages({
          filter: (m) =>
            m.author.id === interaction.user.id && ['hit', 'stand', 'doubledown'].includes(m.content.toLowerCase()),
          max: 1,
          time: 60000,
        })
        .catch(() => {}),
