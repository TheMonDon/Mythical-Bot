...
exports.commandData = new SlashCommandBuilder()
  ...
  .addStringOption((option) => option.setName('input').setDescription('Hex, RGB, name, image URL, or random'))
  .addAttachmentOption((option) => option.setName('attachment').setDescription('An image attachment'));

exports.run = async (interaction) => {
  await interaction.deferReply();

  let input = interaction.options.getString('input');
  const image = interaction.options.getAttachment('attachment')?.url;

  if (image) input = image;
  let color;

  // Create the embed
  const embed = new EmbedBuilder().setTitle('Color Information').setAuthor({
  ...
  embed
    ...
    .setDescription(
      stripIndent(`
        **Name:** ${interaction.client.util.toProperCase(color.css)}
        **Hex:** #${color.hex}
        **Rgb:** rgb(${color.rgb})
        **Hsl:** hsl(${color.hsl})
